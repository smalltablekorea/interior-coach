...
- 수량이 불명확하면 1로 설정`,
  ...
  try {
    ...
    return NextResponse.json(
      { error: "영수증 분석에 실패했습니다. 다시 시도해주세요.", raw: text },
      { status: 422 }
    );
  }
...
export async function PUT(request: NextRequest) {
  ...
  for (const item of items) {
    // 1. Check if material exists in standard catalog
    ...
    if (existingMaterials.length > 0) {
      ...
      // Same item, different price → add note
      ...
      // New material → add to materials table
