...
import { ArrowLeft, Building2, User, Calendar, CheckCircle, Clock, AlertCircle, Pencil, Trash2, Save, X } from "lucide-react";
...
export default function ContractDetailPage() {
  ...
  const handleDelete = async () => {
    try {
      await fetch(`/api/contracts/${id}`, { method: "DELETE" }).catch(() => {});
    } catch {
      // ignore 503
    }
  ...
  const unpaid = contract.contractAmount - paidAmount;
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 text-center">
          ...
          {isEditing ? (
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-[var(--border)] text-white text-sm placeholder:text-[var(--muted)] focus:border-[var(--green)] focus:outline-none text-center text-2xl font-bold"
            />
          ) : (
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 text-center">
          ...
          <p className="text-2xl font-bold text-[var(--red)]">{fmt(unpaid)}</p>
        </div>
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--muted)]">
                      {p.status === "완납"
                        ? `납부일 ${fmtDate(p.paidDate)}${p.method ? ` · ${p.method}` : ""}`
            ...
            {contract.customerName && (
              <div className="flex items-center gap-3">
                ...
                {contract.customerPhone && (
                  <span className="text-[var(--muted)]">{contract.customerPhone}</span>
                )}
              </div>
            )}
